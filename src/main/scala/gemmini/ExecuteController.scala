...
class ExecuteController[T <: Data](xLen: Int, tagWidth: Int, config: GemminiArrayConfig[T])
  ...
  val inputBufferSize = 64 //TODO: put it in configs
  val kernelSize = 4 // TODO
  val padding = 1 //TODO
  ...
  val DoComputes = functs.map(f => f === COMPUTE_AND_FLIP_CMD || f === COMPUTE_AND_STAY_CMD || f === COMPUTE_2D_CONV)
  ...
  val twodConv_len = Reg(UInt(16.W)) // Convolution length
  val k_len = Reg(UInt(8.W))
  ...
  val twodConv = Module(new LineBuffer(inputType, inputType, accType, mesh_tag, tileRows, tileColumns, meshRows, meshColumns, kernelSize, inputBufferSize, padding)) 
  ...
  twodConv.io.b.valid := false.B
  twodConv.io.k.valid := false.B
  twodConv.io.k_len.valid := false.B
  twodConv.io.lb_config.valid := false.B
  twodConv.io.tag_in.valid := false.B
  twodConv.io.out.ready := true.B
  twodConv.io.lb_control := false.B

  twodConv.io.b.bits := DontCare
  twodConv.io.k.bits := DontCare
  twodConv.io.k_len.bits := DontCare
  twodConv.io.lb_config.bits := DontCare
  twodConv.io.tag_in.bits := DontCare
  // Hazards
  ...
  val perform_2d_conv = RegInit(false.B)
  val performing_2d_conv = WireInit(perform_2d_conv && control_state === compute)
  twodConv.io.lb_control := cntl.perform_2d_conv & performing_2d_conv
  // Fire counters which resolve same-bank accesses
  val a_fire_counter = Reg(UInt(16.W)) //Reg(UInt(log2Up(block_size).W))
  val b_fire_counter = Reg(UInt(16.W)) //Reg(UInt(log2Up(block_size).W))
  val d_fire_counter = Reg(UInt(16.W)) //Reg(UInt(log2Up(block_size).W))
  val conv_output_counter = RegInit(0.U(16.W))  
  ...
  val a_row_is_not_all_zeros = Mux(performing_2d_conv, true.B, a_fire_counter < a_rows)
  val b_row_is_not_all_zeros = Mux(performing_2d_conv, true.B, b_fire_counter < b_rows)
  ...
  val Seq(a_valid, b_valid, d_valid) = operands.map { case Operand(addr, start_inputting, counter, started, priority) =>
      ...
      val must_wait_for = ((same_banks zip same_counter) zip (one_ahead zip higher_priorities)).map {
        case ((sb, sc), (oa, hp)) =>
          Mux(performing_2d_conv, (sb && hp && sc), (sb && hp && sc) || oa)
      }

      !must_wait_for.reduce(_ || _)
  }

  a_ready := Mux(performing_2d_conv, twodConv.io.k.ready, true.B)
  b_ready := Mux(performing_2d_conv, twodConv.io.b.ready, true.B)
  ...
  when (!firing) {
	...
	when(performing_2d_conv){
		a_fire_counter := wrappingAdd(a_fire_counter, 1.U, k_len + 1.U)
	}.otherwise{
  ...
  when (!firing) {
	...
	when(performing_2d_conv){
		b_fire_counter := wrappingAdd(b_fire_counter, 1.U, twodConv_len)
	}.otherwise{
  ...
  val about_to_fire_all_rows = Mux(!performing_2d_conv, ((a_fire_counter === (block_size-1).U && a_valid) || a_fire_counter === 0.U) && ((b_fire_counter === (block_size-1).U && b_valid) || b_fire_counter === 0.U) &&((d_fire_counter === (block_size-1).U && d_valid) || d_fire_counter === 0.U) && (a_fire_counter =/= 0.U || b_fire_counter =/= 0.U || d_fire_counter =/= 0.U) && cntl_ready,
	(conv_output_counter === (twodConv_len - 1.U) && twodConv.io.out.fire()))
  ...
  val prev_command_is_conv = RegInit(false.B)
  // FSM logic
  switch (control_state) {
    is (waiting_for_cmd) {
	  ...
	  perform_2d_conv := false.B
	  a_fire_counter := 0.U
	  b_fire_counter := 0.U
	  d_fire_counter := 0.U
	  conv_output_counter := 0.U
      ...
      when(cmd.valid(0))
      {
        ...
        when(DoConfig && !matmul_in_progress && !pending_completed_rob_ids.map(_.valid).reduce(_ || _)) {
		  // rs1[1:0] == 1 ==> config systolic array
		  // rs1[1:0] == 3 ==> config line buffer
		  when(rs1s(0)(1,0) === 1.U){
		  ...
		  }.elsewhen(rs1s(0)(1,0) === 3.U){
			twodConv.io.lb_config.valid := true.B
			twodConv.io.k_len.valid := true.B
			twodConv.io.k_len.bits := rs1s(0)(27,20)
		 	twodConv.io.lb_config.bits.inSize := rs1s(0)(19,4)
			twodConv.io.lb_config.bits.continuous := rs1s(0)(3)
			twodConv.io.lb_config.bits.realKernelSize := rs2s(0)(15,0)

			twodConv_len := rs2s(0)(31,16)
			k_len := rs1s(0)(27,20)
			current_dataflow := Dataflow.WS.id.U
			activation := 0.U
 		  }
        ...
        .elsewhen(DoComputes(0)) {
		  // added 2d conv block
		  when(functs(0) === COMPUTE_2D_CONV){
		  	perform_2d_conv := true.B
			twodConv.io.tag_in.valid := true.B
			twodConv.io.tag_in.bits.rob_id.valid := true.B
			twodConv.io.tag_in.bits.rob_id.bits := cmd.bits(0).rob_id
			twodConv.io.tag_in.bits.addr := b_address_rs2
			twodConv.io.tag_in.bits.cols := b_cols
			twodConv.io.tag_in.bits.rows := b_rows
			prev_command_is_conv := true.B
	      }.otherwise{
    ...
    is (compute) {
	  ...
	  .elsewhen(perform_2d_conv){
		start_inputting_a := true.B // this is for kernel
		start_inputting_b := true.B // this is for input weights
		when(about_to_fire_all_rows){
			cmd.pop := 1.U
			control_state := waiting_for_cmd
  ...
  // Computing logic
  val computing = performing_mul_pre || performing_single_mul || performing_single_preload || performing_2d_conv
  ...
  mesh_cntl_signals_q.io.enq.bits.perform_2d_conv := performing_2d_conv
  ...
  mesh_cntl_signals_q.io.enq.bits.c_addr := Mux(performing_2d_conv, b_address_rs2, c_address_rs2)
  mesh_cntl_signals_q.io.enq.bits.c_rows := Mux(performing_2d_conv, b_rows, c_rows)
  mesh_cntl_signals_q.io.enq.bits.c_cols := Mux(performing_2d_conv, b_cols, c_cols)
  ...
  mesh_cntl_signals_q.io.deq.ready := Mux(!performing_2d_conv, (!cntl.a_fire || mesh.io.a.fire() || !mesh.io.a.ready) && (!cntl.b_fire || mesh.io.b.fire() || !mesh.io.b.ready) && (!cntl.d_fire || mesh.io.d.fire() || !mesh.io.d.ready),
	(!cntl.a_fire || twodConv.io.k.fire() || !twodConv.io.k.ready) && (!cntl.b_fire || twodConv.io.b.fire() || !twodConv.io.b.ready))
  ...
  when (cntl_valid) {
    // Default inputs
    mesh.io.a.valid := Mux(!cntl.perform_2d_conv, cntl.a_fire && dataA_valid, false.B)
    mesh.io.b.valid :=  Mux(!cntl.perform_2d_conv, cntl.b_fire && dataB_valid, false.B)
    mesh.io.d.valid :=  Mux(!cntl.perform_2d_conv, cntl.d_fire && dataD_valid, false.B)
    mesh.io.tag_in.valid :=  Mux(!cntl.perform_2d_conv, true.B, false.B)
	...
	twodConv.io.k.valid := Mux(!cntl.perform_2d_conv, false.B, cntl.a_fire && dataA_valid)
	twodConv.io.b.valid := Mux(!cntl.perform_2d_conv, false.B, cntl.b_fire && dataB_valid)

	twodConv.io.k.bits := dataA.asTypeOf(Vec(meshColumns, Vec(tileColumns, inputType)))
	twodConv.io.b.bits := dataB.asTypeOf(Vec(meshColumns, Vec(tileColumns, inputType)))
  ...
  // Scratchpad writes
  val w_address = Mux(performing_2d_conv, twodConv.io.tag_out.addr, mesh.io.tag_out.addr)
  ...
  val current_w_bank_address = Mux(current_dataflow === Dataflow.WS.id.U, Mux(performing_2d_conv, w_row + conv_output_counter, w_row + output_counter.value),
    w_row + block_size.U - 1.U - output_counter.value)

  val is_garbage_addr = w_address.is_garbage()

  val w_matrix_rows = Mux(performing_2d_conv, twodConv.io.tag_out.rows, mesh.io.tag_out.rows)
  val w_matrix_cols = Mux(performing_2d_conv, twodConv.io.tag_out.cols, mesh.io.tag_out.cols)

  val write_this_row = Mux(performing_2d_conv, true.B, Mux(current_dataflow === Dataflow.WS.id.U, output_counter.value < w_matrix_rows,
  ...
  for(i <- 0 until sp_banks) {
    ...
    io.srams.write(i).data := Mux(performing_2d_conv, twodConv.io.out.bits.asUInt() ,activated_wdata.asUInt())
  ...
  when(twodConv.io.out.fire()){
	when(conv_output_counter <  (twodConv_len - 1.U)){
		conv_output_counter := conv_output_counter + 1.U
	}.elsewhen(conv_output_counter === (twodConv_len - 1.U) && control_state === compute){
		io.completed.valid := true.B
		io.completed.bits := twodConv.io.tag_out.rob_id.bits
	}
	
	start_array_outputting := true.B & performing_2d_conv
  }
  ...
  */
  ...
  when (!mesh_completed_rob_id_fire && !performing_2d_conv && !prev_command_is_conv){
