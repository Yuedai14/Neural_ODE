...
/*class VecMul_unit[T <: Data](inputType: T, outputType: T, vecSize: Int)(implicit ev: Arithmetic[T]) extends Module{
	import ev._
...
class LineBufferUnitConfig[T <: Data : Arithmetic](inputSize: Int, kernelSize: Int) extends Bundle{
...
class LineBufferUnit[T <: Data : Arithmetic, U <: Data]
  ...
  when(io.k.fire() & io.lb_control & convSeqWritten){
	if(meshColumns < kernelSize){
    	io.k.bits.zipWithIndex.foreach{case(k, i) => kernelMatrix(kernelStorePointer + i.U) := k(0)}
	}else{
		kernelMatrix.zipWithIndex.foreach{case(k, i) => k := io.k.bits(i)(0)}
	}
