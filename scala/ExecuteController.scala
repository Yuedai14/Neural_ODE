...
class ExecuteController[T <: Data](xLen: Int, tagWidth: Int, config: GemminiArrayConfig[T])
  ...
  twodConv.io.lb_config := cntl.perform_2d_conv & performing_2d_conv
  ...
  when (!firing) {
	...
	when(performing_2d_conv){
		a_fire_counter := wrappingAdd(a_fire_counter, 1.U, k_len)
	}.otherwise{
  ...
  val about_to_fire_all_rows = Mux(!performing_2d_conv, ((a_fire_counter === (block_size-1).U && a_valid) || a_fire_counter === 0.U) && ((b_fire_counter === (block_size-1).U && b_valid) || b_fire_counter === 0.U) &&((d_fire_counter === (block_size-1).U && d_valid) || d_fire_counter === 0.U) && (a_fire_counter =/= 0.U || b_fire_counter =/= 0.U || d_fire_counter =/= 0.U) && cntl_ready,
	a_fire_counter === (k_len-1.U) && b_fire_counter === (twodConv_len - 1.U) && (conv_output_counter === (twodConv_len - 1.U) && twodConv.io.out.fire()))
  ...
  when (cntl_valid) {
	...
	twodConv.io.tag_in.valid := Mux(!cntl.perform_2d_conv, false.B, true.B)
	
	twodConv.io.tag_in.bits.rob_id := cntl.rob_id
	twodConv.io.tag_in.bits.addr := cntl.c_addr
	twodConv.io.tag_in.bits.cols := cntl.c_cols
	twodConv.io.tag_in.bits.rows := cntl.c_rows
  }
  ...
  when(twodConv.io.out.fire()){
	...
	}.elsewhen(conv_output_counter === (twodConv_len - 1.U)){
		io.completed.valid := true.B
  ...
  */
  ...
  when (!mesh_completed_rob_id_fire && !performing_2d_conv) {
