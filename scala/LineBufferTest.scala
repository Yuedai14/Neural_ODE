...
// line buffer tester
class LineBufferTester(c: LineBuffer[SInt, UInt], inputSeq: Seq[Seq[Int]], kernel: Seq[Seq[Int]], inSize: Int, realKernelSize: Int, continuous: Boolean, goldResult: Seq[Seq[Int]], verbose: Boolean = false) extends PeekPokeTester(c)
{
	println("Begin test")
	val maxWait = 100
...
object SIntLineBufferTester{
	def apply(meshRows: Int, meshColumns: Int, kernelSize: Int, inputBufferSize: Int, padding: Int, inputSeq: Seq[Seq[Int]], kernel: Seq[Seq[Int]], inSize: Int, realKernelSize: Int, continuous: Boolean, goldResult: Seq[Seq[Int]]): Boolean = {
		iotesters.Driver.execute(Array("--backend-name", "treadle", "--generate-vcd-output", "on"), () => new LineBuffer(SInt(16.W), SInt(16.W), SInt(16.W), UInt(32.W), 1, 1, meshRows, meshColumns, kernelSize, inputBufferSize, padding)) {
