...
#define gemmini_extended_compute_conv(K, B, K_cols, K_rows, B_cols, B_rows) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, ((uint64_t)(K_rows) << (ADDR_LEN + 16)) | ((uint64_t)(K_cols) << ADDR_LEN) | (uint64_t)(K), ((uint64_t)(B_rows) << (ADDR_LEN + 16)) | ((uint64_t)(B_cols) << ADDR_LEN) | (uint64_t)(B), k_COMPUTE_CONV)
...
#define gemmini_compute_conv(K, B) \
  gemmini_extended_compute_conv(K, B, DIM, DIM, DIM, DIM)
...
#define gemmini_config_conv(k_len, conv_len, in_size, kernel_size, continuous) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, ((uint64_t)(k_len) << 20) | ((uint64_t)(in_size) << 4) | (continuous << 3) | CONFIG_CONV, ((uint64_t)(conv_len) << 16) | ((uint64_t)(kernel_size)), k_CONFIG)
...
// 2d convolution load data functions
static void load_conv(const elem_t * K, const elem_t * B, size_t k_len, size_t conv_len){
	const uint32_t K_sp_addr_start = 0;
	const uint32_t B_sp_addr_start = BANK_NUM * BANK_ROWS / 2;

	// Move-in B
	gemmini_config_ld(conv_len * sizeof(elem_t));
	for(size_t i = 0; i < conv_len; i++){
		const elem_t * const B_dram_addr = B + i*DIM;
		const uint32_t B_sp_addr = B_sp_addr_start + i*DIM;

		gemmini_extended_mvin(B_dram_addr, B_sp_addr, DIM, DIM);
	}

	// Move-in K
	gemmini_config_ld(sizeof(elem_t));
	const int max_time = k_len < DIM ? 1 : (k_len/DIM + 1);
	for(size_t i = 0; i < max_time; i++){
		const elem_t * const K_dram_addr = K + i * DIM;
		const uint32_t K_sp_addr = K_sp_addr_start + i * DIM;

		gemmini_extended_mvin(K_dram_addr, K_sp_addr, DIM, DIM);
	}
